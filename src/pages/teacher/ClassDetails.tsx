    ...
    <div className="space-y-3">
      ...
      <div className="card overflow-x-auto p-0">
        <table className="min-w-full divide-y divide-line text-sm">
          <thead className="bg-surface-2 text-left text-xs uppercase tracking-wide text-content-subtle">
            <tr>
              <th className="sticky left-0 z-10 bg-surface-2 px-4 py-3 shadow-[2px_0_0_0_rgb(var(--line))]">
                Student
              </th>
              {items.map((i) => {
                const cat = categories.find((c) => c.id === i.category_id);
                return (
                  <th key={i.id} className="min-w-[150px] px-3 py-3 text-center">
                    <div className="text-sm font-semibold normal-case text-content">{i.title}</div>
                    <div className="mt-1 flex items-center justify-center gap-1.5 text-[11px] font-normal normal-case">
                      <span className="text-content-subtle">{cat?.name}</span>
                      <span className="text-content-subtle">·</span>
                      <span className="font-mono text-content-muted">/{i.max_score}</span>
                      <span
          ...
          <tbody className="divide-y divide-line">
            {roster.map((r) => (
              <tr key={r.student_id}>
                <td className="sticky left-0 z-10 bg-surface px-4 py-3 font-medium shadow-[2px_0_0_0_rgb(var(--line))]">
                  {r.full_name}
                </td>
                {items.map((i) => {
                  const key = `${r.student_id}|${i.id}`;
                  ...
                  const showStatusPicker = (cell?.status ?? 'graded') !== 'graded';
                  return (
                      ...
                      <div className="flex flex-col gap-1.5">
                        <div className="flex items-center gap-1.5">
                          <input
                            ...
                            className={`input h-10 flex-1 text-center text-base font-medium ${
                              cell?.error
                          ...
                          />
                          <span className="select-none font-mono text-xs text-content-subtle">
                            / {i.max_score}
                          </span>
                        </div>

                        <div className="flex items-center justify-between gap-1">
                          {showStatusPicker || locked ? (
                            <select
                            ...
                            <button
                              type="button"
                              onClick={() => update(key, { status: 'missing' })}
                              className="rounded-md px-1.5 py-0.5 text-[11px] text-content-subtle transition hover:bg-surface-3 hover:text-content"
                              title="Mark missing / late / excused"
                            >
                              Mark…
                            </button>
                          )}
                        ...
                        {pending ? (
                          <div
                            className="flex items-center gap-1 text-[11px] font-medium text-amber-700 dark:text-amber-300"
                          ...
                          <div className="text-[11px] text-content-subtle">Saving…</div>
                        ) : cell?.saved ? (
      ...
      <p className="text-xs text-content-subtle">
        Tip: cells with an amber ring are unsaved — they save automatically when you tab away. Use
        the buttons at the top to bulk save as draft or publish. Cells in a finalized period are
        locked; editing them opens a request-change dialog that goes to the Department Head for
        approval.
      </p>
