...
-- Resolve current auth.uid() to a role text (or null)
create or replace function public.current_role()
returns text language sql security definer stable as $$
  select role from public.users where id = auth.uid();
$$;

-- Resolve current auth.uid() to student.id (or null)
create or replace function public.current_student_id()
returns uuid language sql security definer stable as $$
  select id from public.students where user_id = auth.uid();
$$;

-- Resolve current auth.uid() to teacher.id (or null)
...
-- =============================================================================
-- TRIGGER: when auth.users created, create public.users row with role=pending
-- =============================================================================
...
-- =============================================================================
-- TRIGGER: generate unique class_code on insert
-- =============================================================================
...
begin
  ...
  loop
    -- 6-char uppercase alphanumeric (excluding ambiguous chars)
    candidate := upper(substring(replace(replace(replace(encode(gen_random_bytes(6), 'base64'), '/', ''), '+', ''), '=', '') from 1 for 6));
    -- Remove ambiguous chars (O, 0, I, 1) by replacement
...
-- =============================================================================
-- TRIGGER: set_updated_at — generic
-- =============================================================================
...
-- =============================================================================
-- TRIGGER: log score changes to audit_logs
-- =============================================================================
...
-- =============================================================================
-- TRIGGER: log finalized_grades to audit_logs
-- =============================================================================
...
-- =============================================================================
-- TRIGGER: notify student on score change (when not draft)
-- =============================================================================
...
declare
  ...
  v_item_title text;
begin
  -- Skip notification for draft scores
...
-- =============================================================================
-- TRIGGER: prevent score edits for locked periods
-- =============================================================================
...
-- =============================================================================
-- RPC: convert_percentage_to_numeric (PH 1.0-5.0)
-- =============================================================================
...
-- =============================================================================
-- RPC: compute_final_grade — average of midterm + finals
-- =============================================================================
...
-- =============================================================================
-- RPC: compute_gpa — weighted by units across student's classes in a term
-- =============================================================================
...
-- =============================================================================
-- RPC: finalize_period — locks all enrolled students' computed grades
-- =============================================================================
...
create policy items_select on public.grade_items for select
  using (
    public.current_role() = 'admin'
    or exists (select 1 from public.grade_categories gc
               join public.classes c on c.id = gc.class_id
               where gc.id = grade_items.category_id
                 and (c.teacher_id = public.current_teacher_id()
                      or (grade_items.is_published = true
...
create policy scores_select on public.scores for select
  using (
    public.current_role() = 'admin'
    or exists (select 1 from public.grade_items gi
               join public.grade_categories gc on gc.id = gi.category_id
               join public.classes c on c.id = gc.class_id
               where gi.id = scores.grade_item_id
                 and (c.teacher_id = public.current_teacher_id()
                      or (scores.student_id = public.current_student_id()
                          and scores.is_draft = false
                          and gi.is_published = true)))
  );
...
create policy finalized_select on public.finalized_grades for select
  using (
    ...
    or student_id = public.current_student_id()
    or exists (select 1 from public.classes c
               where c.id = finalized_grades.class_id
                 and c.teacher_id = public.current_teacher_id())
  );

drop policy if exists finalized_admin_all on public.finalized_grades;
create policy finalized_admin_all on public.finalized_grades for all
  using (public.current_role() = 'admin')
  with check (public.current_role() = 'admin');
...
create policy appeals_select on public.appeals for select
  using (
    public.current_role() = 'admin'
    or student_id = public.current_student_id()
    or exists (select 1 from public.scores sc
               join public.grade_items gi on gi.id = sc.grade_item_id
               join public.grade_categories gc on gc.id = gi.category_id
               join public.classes c on c.id = gc.class_id
               where sc.id = appeals.score_id
                 and c.teacher_id = public.current_teacher_id())
  );
